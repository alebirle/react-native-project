...
const Cell = (props) => {
   ...
   const { player, setPlayer, board, setBoard, gameWon, setGameWon, row, col } = props;
   ...
   const checkForWinner = () => {
      if (board[1][1].length > 0 && (board[0][0] == board[1][1] && board[1][1] == board[2][2] || board[2][0] == board[1][1] && board[1][1] == board[0][2])) {
         return true;
      }

      for (var i=0; i<3; i++) {
         if (board[i][0].length > 0 && board[i][0] == board[i][1] && board[i][1] == board[i][2]) {
            return true;
         }

         if (board[0][i].length > 0 && board[0][i] == board[1][i] && board[1][i] == board[2][i]) {
            return true;
         }
      }
...
const Game = ({navigation}) => {
   ...
   return (
      <View>
         <Text style = {styles.myState}>{gameWon ? 'Game over!' : ''}</Text>
